...
func (s *SessionRoutes) SetupSessionRoutes(router *gin.RouterGroup) {
	sessionGroup := router.Group("/quizzes")
	{
		...
		sessionGroup.PATCH("/:quizId/sessions/:sessionId", s.AuthMiddleware.Verify, s.SessionController.EndQuizSession)
		sessionGroup.GET("/:quizId/sessions/:sessionId", s.AuthMiddleware.Verify, s.SessionController.GetInfoAboutSession)
